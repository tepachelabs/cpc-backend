    ...
    for product_reminder in reminders:
        ...
        if product is not None:
            if len(product.variants) == 1:
                messages.append(
                    f"> {telegram.parse_text(str(product.variants[0].inventory_quantity))} \\-\\-\\-\\> {product.title}"
                )
            else:
                message = f"🏷️ *{product.title}*:\n"
                for variant in product.variants:
                    message += f"> {telegram.parse_text(str(variant.inventory_quantity))} \\-\\-\\-\\> {variant.title}\n"
